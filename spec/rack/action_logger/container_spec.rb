...
RSpec.describe Rack::ActionLogger::Container do
  ...
  describe 'is_emit_started' do
    it 'should return false as default' do
      expect(described_class.is_emit_started).to be_falsy
    end

    it 'should return true after set' do
      described_class.is_emit_started = true
      expect(described_class.is_emit_started).to be_truthy
    end
  end

  describe 'append_log' do
    it 'should return empty hash as default' do
      expect(described_class.append_logs).to eq []
    end

    it 'can append log' do
      described_class.add_append_log(hash, tag)
      expected = [hash.merge({ tag: tag })]
      expect(described_class.append_logs).to eq expected
    end

    it 'can be the same object after changed' do
      append_log = described_class.append_logs
      described_class.add_append_log(hash, tag)
      expect(described_class.append_logs).to be append_log
    end

    it 'should log with nil tag' do
      expected = [hash.merge({ tag: nil })]
      described_class.add_append_log(hash)
      expect(described_class.append_logs).to eq expected
    end

    it 'should not log without invalid tag' do
      described_class.add_append_log(hash, ['test'])
      expect(described_class.append_logs).to eq []
    end

    it 'should not log without invalid hash' do
      described_class.add_append_log(nil, tag)
      expect(described_class.append_logs).to eq []
    end
  end
  ...
  describe 'attributes' do
    ...
    it 'can merge attributes' do
      ...
      expect(described_class.attributes).to eq hash
    end

    it 'can be the same object after changed' do
      attributes = described_class.attributes
      described_class.merge_attributes(hash)
      expect(described_class.attributes).to be attributes
    end
  ...
  describe 'request_log' do
    it 'should return empty hash as default' do
      expect(described_class.request_log).to be {}
    end

    it 'can get request log' do
      described_class.set_request_log(hash, tag)
      expected = hash.merge({ tag: tag })
      expect(described_class.request_log).to eq expected
    end
